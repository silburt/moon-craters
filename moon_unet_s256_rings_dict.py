...
def train_and_test_model(X_train,Y_train,X_valid,Y_valid,X_test,Y_test,loss_data,loss_csvs,MP,i_MP):
    
    # params
    lmbda, drop = MP['lmbda'][i_MP], MP['dropout'][i_MP]        # iterating params
    dim, learn_rate, nb_epoch, bs = MP['dim'], MP['lr'], MP['epochs'], MP['bs']
    FL, init, n_filters = MP['filter_length'], MP['init'], MP['n_filters']
    ...
    print '\nTest Score is %f \n'%model.evaluate(X_test.astype('float32'))
...
def run_cross_validation_create_models(dir,MP):
    ...
    valid_data, valid_target = valid_data[:n_train], valid_target[:n_train]
    test_data, test_target = test_data[:n_train], test_target[:n_train]
    ...
    MP['lmbda']=[0,1e-6,1e-5,0,1e-6,1e-5,0,1e-6,1e-5]               #regularization
    MP['dropout']=[0.15,0.15,0.15,0.25,0.25,0.25,0.35,0.35,0.35]    #dropout after merge layers

    #Iterate
    for i in range(N_runs):
        train_and_test_model(train_data,train_target,valid_data,valid_target,test_data,test_target,loss_data,loss_csvs,MP,i)
