...
def run_cross_validation_create_models(MP):
    ...
    train_data=np.load('%s/train_0_input.npy'%dir).reshape((30016,256,256,1))
    train_target=np.load('%s/train_0_targets.npy'%dir)
    #train_ids = np.load('%s/Train_rings/train_ids.npy'%dir)
    valid_data=np.load('%s/dev_0_input.npy'%dir).reshape((30016,256,256,1))
    valid_target=np.load('%s/dev_0_targets.npy'%dir)
    #valid_ids = np.load('%s/Dev_rings/dev_ids.npy'%dir)
    test_data=np.load('%s/test_0_input.npy'%dir).reshape((30016,256,256,1))
