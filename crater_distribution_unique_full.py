...
if __name__ == '__main__':
    ...
    for llt2,rt2 in thresholds:
        ...
        pred_output_name = '%s/%s_uniquepred_llt%.1e_rt%.1e_n%d.npy'%(dir,datatype,llt2,rt2,len(pred))   #save directory
        ...
        for ilen_dir in ilen_dirs:
            ...
            pred_crater_dist = extract_unique_pred(pred_crater_dist, ilen_pred, ilen_id, ilen_P, llt2, rt2, pred_output_name)

    print "getting ground truth"
    GT_output_name = '%s/%s_uniqueGT_llt%.1e_rt%.1e_n%d.npy'%(dir,datatype,llt2,rt2,len(pred))   #save directory
    GT_crater_dist = np.empty([0,3])
