...
def get_recall(dir, n_samples, dim, model, X, Y, ids, beta=1):
    ...
    for i in range(n_samples):
        if len(csvs[i]) < 3:    #exclude csvs with tiny crater numbers
            continue
        N_match, N_csv, N_templ, maxr, csv_dupe_flag = template_match_target_to_csv(Y_pred[i], csvs[i])
        if N_match > 0:
...
def train_and_test_model(X_train,Y_train,X_valid,Y_valid,X_test,Y_test,ID_valid,ID_test,MP,i_MP):
    ...
    if MP['save_models'] == 1:
        model.save('models/unet_s256_rings_n112_L%.1e_D%.2f_cw.h5'%(lmbda,drop))
