...
def train_and_test_model(X_train,Y_train,X_valid,Y_valid,X_test,Y_test,n_train_samples,learn_rate,batch_size,lmbda,FL,nb_epoch,im_width,im_height,rs,save_model):
    ...
    if save_model == 1:
        model.save('models/ds4_sigmoid_FL%d.h5'%FL)

    test_pred = model.predict(X_test.astype('float32'), batch_size=batch_size, verbose=2)
    npix = Y_test.shape[0]*Y_test.shape[1]*Y_test.shape[2]
    return np.sum((test_pred - Y_test)**2)/npix    #calculate test score
